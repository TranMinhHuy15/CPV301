...
# Multi-seed support: same TOP_SEED env var as cell15_train_cached_5f.py,
# so this can be run right after each seed's training without edits.
SEED        = int(os.environ.get("TOP_SEED", "42"))
SUFFIX      = f"_seed{SEED}"
CKPT_PATH   = os.path.join(OUTPUT_DIR, f"best_cached{SUFFIX}.pth")
BATCH_SIZE  = 8
NUM_WORKERS = 4
...
def low_far_metrics(targets, scores, target_far=TARGET_FAR):
    ...
    if np.any(mask):
        ...
        # mauc, Recall@FAR<=0.1, chosen Threshold, actual FAR achieved at that threshold
...
for lead_time, h_idx in LEAD_TO_HIDX.items():
    ...
    mauc01, recall01, thr01, far01 = low_far_metrics(targets, score_matched)
...
print(f"\n{'='*65}")
print(f"  TOP BASELINE v5-5f (cached, seed={SEED}) — FINAL EVALUATION RESULTS (RQ1)")
print(f"{'='*65}")
...
with open(os.path.join(OUTPUT_DIR, f"eval_paper_style{SUFFIX}.txt"), "w") as f:
...
results_path = os.path.join(OUTPUT_DIR, f"eval_results_rq1_top5f{SUFFIX}.txt")
with open(results_path, "w") as f:
    f.write(f"TOP Baseline v5-5f (cached, seed={SEED}) - Evaluation Results (RQ1)\n")
