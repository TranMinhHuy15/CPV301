...
|---|---|---|---|---|
| Random-offset | round(d·(N−1)), d ~ U(0.1, 0.9) → 1–10 steps | 0.5–5.0 s (mean 2.75 s) | baseline; the published objective, the RQ1 anchor and RQ2 condition D | 3 (seeds 42/43/44) |
| FixedLag τ = 1.0 s | 2 | 1.0 s | **main RQ3 condition** ("RiskProp fixed") | 3 |
...
The earlier summary (`RESULTS.md` §5.3) reported lower seed variance as the main effect of fixed-lag training. The sensitivity runs let us test whether that holds beyond τ = 1.0 s.
...
- **Change from the earlier summary.** `RESULTS.md` §5.3 said that the main measurable effect of fixed-lag training is lower seed variance. The τ-sensitivity runs and the official test do not support that (Finding 4), and this document replaces that statement. `RESULTS_RQ2.md` Finding 5 was worded to match.
- **Training sessions.** The random-offset and τ = 1.0 s runs were trained in an earlier session (105–148 s per epoch). The τ = 0.5 / 1.5 s runs were trained later on one RTX 4080 (about 92 s per epoch). Code, data, split and hyperparameters are identical, but GPU non-determinism across sessions may add some run-to-run variation.
...
2. **Rút lại kết luận "fixed-lag giảm phương sai giữa các seed".** `RESULTS.md` §5.3 cũ lấy đây làm kết quả chính. Với τ = 1.5 s (SD 0.030 ≈ random 0.033) và trên official test (0.013 so với 0.015), kết luận này không còn đứng được. Đề xuất: bỏ hẳn, chỉ ghi "không kết luận được với 3 seed". Nhóm đồng ý không? `RESULTS_RQ2.md` (Finding 5 và ghi chú số 7) đã được sửa cho khớp.
